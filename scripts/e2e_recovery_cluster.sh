...
./build/rstone-cli --endpoint 127.0.0.1:8081 put recovery:key before
expect_value recovery:key before

stop_component gateway
start_gateway
expect_value recovery:key before
./build/rstone-cli --endpoint 127.0.0.1:8081 put recovery:after-gateway after-gateway
expect_value recovery:after-gateway after-gateway

stop_component pd
start_pd
status_after_pd="$(./build/rstone-cli --endpoint 127.0.0.1:8081 status)"
...
expect_value apple red
expect_value zebra stripe
./build/rstone-cli --endpoint 127.0.0.1:8081 put yak large
expect_value yak large

status_after_store="$(./build/rstone-cli --endpoint 127.0.0.1:8081 status)"
